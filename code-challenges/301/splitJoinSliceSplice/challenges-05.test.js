...
const howMuchPencil = (str) => {
  ...
  for(let i=0; i<str.length; i++) {
    var slicedStr = str.slice(i, str.length);
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(ingredient => {
    let splitArr = ingredient.split(' ');
    let fooArr = splitArr.splice(2, 3);
    result.push(fooArr.join(' '));
  })
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(item => {
    let makeArr = item.split(' ');
    let arrWords = makeArr.splice(0, 1);
    arrWords.forEach(arr => {
...
const removeVowels = (str) => {
  ...
  //can replace the entire function with one line of regex
...
const extractVowels = (str) => {
  ...
  finalArr.push(noVowels.join(''));
  finalArr.push(vowels.join(''));
...
//   let finalArr = [];
//   let noVowelsStr = str.replace(/[aeiou]/g, '');
...
//   finalArr.push(noVowelsStr);
//   finalArr.push(vowelArr.join(''))
//   return finalArr;
