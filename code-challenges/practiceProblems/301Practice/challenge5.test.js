...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    let newIndex = ingredient.indexOf(' ')+1;
    newArr.push(ingredient.slice(newIndex));
  });
  return newArr;
};
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    newArr.push(ingredient.slice(2, ingredient.length).join(',').replace(/,/g, ' '));
