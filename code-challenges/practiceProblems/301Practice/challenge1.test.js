...
const addNumbers = (num, arr, numTimes, callback) => {
...
function fizzbuzz(arr){
  let output = [];
  arr.forEach(item => {
    if(item % 15 === 0){ output.push('Fizz Buzz');} else
    if(item % 3 === 0){ output.push('Fizz'); } else
    if(item % 5 === 0){ output.push('Buzz'); } else {
      output.push(item);
    }
  });
  return output;
